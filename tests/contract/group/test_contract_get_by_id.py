...
class TestContractgetByID:
    ...
    def test_access_denied_status_code(self, uni_readiness_check, uni_service_anon):
        response, _ = self.test_access_denied(uni_service_anon)
    ...
    def test_access_denied_response(self, uni_service_anon):
        _, response_model_obj = self.test_access_denied(uni_service_anon)
    ...
    def test_not_autorized_status_code(self, uni_service_wrong_creds):
        response, _ = self.test_not_autorized(uni_service_wrong_creds)
        assert response.status_code == 401, f"Expected status code 401, got {response.status_code}"
    def test_not_autorized_response(self, uni_service_wrong_creds):
        _, response_model_obj = self.test_not_autorized(uni_service_wrong_creds)
    ...
    def test_search_success_status_code(self, uni_service): 
        response, _, _ = self.test_search_success(uni_service)   
        assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
        
    def  test_search_success_response(self, uni_service, clean_group_uni):   
        _, response_model_obj, group = self.test_search_success(uni_service)
